...
app.config["FILE_UPLOADS"] = "D:\\Huffman\\uploads"
...
def encrypt():
    if request.method == "GET":
        ...
        if len(up_file.filename) > 0:
            ...
            ftype = "-encrypted.huf"
            while True:
                if 'D:\\Huffman\\uploads\\{}-encrypted.huf'.format(filename) in glob.glob('D:\\Huffman\\uploads\\*-encrypted.huf'):
                    print('hello')
                    os.system('move D:\\Huffman\\uploads\\{}-encrypted.huf downloads/'.format(filename))
                    print('beforeBreak')
                    break
